  ...
  <div style="height:100%;">
    ...
    <div class="digit" >
      ...
      <ul class="wei_ul">
        ...
        <li class="one-li" @click="open_btn(1)">红一</li>
        <!-- <router-link >万位</router-link> -->
        <li class="two-li" @click="open_btn(2)">红二</li>
        <li class="three-li" @click="open_btn(3)">红三</li>
        <li class="four-li" @click="open_btn(4)">红四</li>
        <li class="five-li" @click="open_btn(5)">红五</li>
        <li class="six-li" @click="open_btn(6)">蓝一</li>
        <li class="seven-li" @click="open_btn(7)">蓝二</li>
      </ul>>
            ...
            <div class="start" v-show="isShowSelect_wei">
                ...
                <div class="start-title">
                    ...
                    <tr class="start-contents1-title" v-for="(items,indexs) in start_list_box" v-if="indexs==0" style="display:flex;display: -webkit-flex;">
                        ...
                        <td class="title-width" style="flex:3;-webkit-flex  : 3;" v-for="(item,index) in items.titles" v-if="index == 1">{{item}}</td>
                        <td v-for="(item,index) in items.titles" v-if="index == 2" style="flex:1;-webkit-flex  : 1;">
                        {{item}}
                        </td>

                        <td v-for="(item,index) in items.titles" v-if="index == 3" style="flex:1;-webkit-flex  : 1;">
                        {{item}}
                        </td>
                ...
                <div class="start-body">
                    ...
                    <tr v-for="(items,indexs) in start_list_box" v-if="(indexs!= 30 && indexs != 60 && indexs != 100 && indexs != 200)" style="display:flex;display: -webkit-flex;">
                        ...
                        <span v-for="(item,index) in items.res" v-if="index < 5" style="margin-right:0.1rem">{{item}}</span>
                        <span v-for="(item,index) in items.res" v-if="index > 5" style="margin-right:0.05rem;color:#35b5fe">{{item}}</span>
                        </td>
                        <td style="flex:1;-webkit-flex  : 1;">{{items.span}}</td>
                        <td style="flex:1;-webkit-flex  : 1;">
                            {{items.duplicate_num}}
                        </td>
                    </tr>
                    </table>
                ...
                <div class="periods">
                    ...
                    <div class="wan_table">
                    <ul >
                        <li class="periods1">期数</li>
                        <li v-for="n in 35 " > {{ n }}</li>
                    </ul>
                    </div>
                    <div class="wan_list" ref="wan_list">
                        <!-- 左边表格期数 -->
                        <div class="wan_table_2" style="flex:1.94;-webkit-flex  : 1.94;">
                    ...
                    <div class="bottom_list" style="margin-bottom: 1.09rem;width:25rem;display:flex;display: -webkit-flex;">
                    ...
                    <ol v-for="(items,indexs) in bottom_list" v-if="indexs < 36" style="flex:1;-webkit-flex  : 1;">
                ...
                <div class="periods">
                    ...
                    <div class="bottom_list" style="margin-bottom: 1.09rem;width:25rem;display:flex;display: -webkit-flex;">
                        ...
                        <ol v-for="(items,indexs) in bottom_list_2" v-if="indexs < 36" style="flex:1;-webkit-flex  : 1;">
                        <li>{{items.total_appear}}</li>
                        <li>{{items.continue_max}}</li>
                        <li>{{items.omit_max}}</li>
                        <li>{{ Math.floor(items.omit_mean)}}</li>
                ...
                <div class="periods">
                    ...
                    <div class="bottom_list" style="margin-bottom: 1.09rem;width:25rem;display:flex;display: -webkit-flex;">
                        ...
                        <ol v-for="(items,indexs) in bottom_list_3" v-if="indexs < 36" style="flex:1;-webkit-flex  : 1;">
                        <li>{{items.total_appear}}</li>
                        <li>{{items.continue_max}}</li>
                        <li>{{items.omit_max}}</li>
                        <li>{{ Math.floor(items.omit_mean)}}</li>
                ...
                <div class="periods">
                    ...
                    <div class="bottom_list" style="margin-bottom: 1.09rem;width:25rem;display:flex;display: -webkit-flex;">
                        ...
                        <ol v-for="(items,indexs) in bottom_list_4" v-if="indexs < 36" style="flex:1;-webkit-flex  : 1;">
                        <li>{{items.total_appear}}</li>
                        <li>{{items.continue_max}}</li>
                        <li>{{items.omit_max}}</li>
                        <li>{{ Math.floor(items.omit_mean)}}</li>
                ...
                <div class="periods">
                    ...
                    <div class="bottom_list" style="margin-bottom: 1.09rem;width:25rem;display:flex;display: -webkit-flex;">
                        ...
                        <ol v-for="(items,indexs) in bottom_list_5" v-if="indexs < 36" style="flex:1;-webkit-flex  : 1;">
                        <li>{{items.total_appear}}</li>
                        <li>{{items.continue_max}}</li>
                        <li>{{items.omit_max}}</li>
                        <li>{{ Math.floor(items.omit_mean)}}</li>
                ...
                <div class="periods">
                    <!-- 表头 -->
                        ...
                        <ol v-for="(items,indexs) in bottom_list_6" v-if="indexs < 36" style="flex:1;-webkit-flex  : 1;">
                            <li>{{items.total_appear}}</li>
                            <li>{{items.continue_max}}</li>
                            <li>{{items.omit_max}}</li>
                            <li>{{ Math.floor(items.omit_mean)}}</li>
                ...
                <div class="periods">
                    <!-- 表头 -->
                        ...
                        <ol v-for="(items,indexs) in bottom_list_7" v-if="indexs < 36" style="flex:1;-webkit-flex  : 1;">
                            <li>{{items.total_appear}}</li>
                            <li>{{items.continue_max}}</li>
                            <li>{{items.omit_max}}</li>
                            <li>{{ Math.floor(items.omit_mean)}}</li>
  ...
  methods: {
    ...
    //   document.getElementById('app').style.overflow='hidden';
      requestOpt.reqnoGetpara(
        ...
        {
          params: {
            lot_type_id: this.$route.params.id,
        ...
        res => {
          ...
          this.wan_data_list  = this.translate_arr(this.arr_list.stat[1].omits,this.len);
          this.qian_data_list  = this.translate_arr(this.arr_list.stat[2].omits,this.len);
          this.bai_data_list  = this.translate_arr(this.arr_list.stat[3].omits,this.len);
          this.shi_data_list  = this.translate_arr(this.arr_list.stat[4].omits,this.len);
          this.ge_data_list  = this.translate_arr(this.arr_list.stat[5].omits,this.len);
          this.lan1_data_list  = this.translate_arr(this.arr_list.stat[6].omits,this.len);
          this.lan2_data_list  = this.translate_arr(this.arr_list.stat[7].omits,this.len);
          ...
          this.bottom_list_7 = this.arr_list.stat[7];
           ...
           document.querySelector(".mint-loadmore-top").style.color = "#666";
            ...
            setTimeout(function() {
                that.draw_cvs(that.len,'.wan_table_3','#my_draw_wan','#canvas_wan',that.wan_data_list);
            }, 50);

            setTimeout(function() {
            that.draw_cvs(that.len,'.qian_table_3','#my_draw_qian','#canvas_qian',that.qian_data_list);
            }, 50);

            setTimeout(function() {
                that.draw_cvs(that.len,'.bai_table_3','#my_draw_bai','#canvas_bai',that.bai_data_list);
            }, 50);

            setTimeout(function() {
                that.draw_cvs(that.len,'.shi_table_3','#my_draw_shi','#canvas_shi',that.shi_data_list);
             }, 50);

            setTimeout(function() {
                that.draw_cvs(that.len,'.ge_table_3','#my_draw_ge','#canvas_ge',that.ge_data_list);
            }, 50);

            setTimeout(function() {
                that.draw_cvs(that.len,'.lan1_table_3','#my_draw_lan1','#canvas_lan1',that.lan1_data_list);
            }, 50);

            setTimeout(function() {
                that.draw_cvs(that.len,'.lan2_table_3','#my_draw_lan2','#canvas_lan2',that.lan2_data_list);
            }, 50);
      ...
      requestOpt.reqnoGetpara(
        "lottery_results_and_forms/" + this.$route.params.id,
        {
          params: {
            list_length: this.my_num
          }
...
// .wan_table_3  table tr td:nth-child(1){
//   width:1rem;
// }
