    ...
    <div class="cathectic-alert">
      <div class="cathectic-alert-content"  v-show="isShowTable">
        <table class="zs_table" style="over-flow:hidden" v-for="(items,indexs) in arr_list" v-if="receiveValue == 0 && indexs == 1">
          ...
          <tr  v-for="(item,index) in NameList" v-if="$route.params.id == item.id">
      ...
      <div class="select_list" v-if="isShowList" :class="{spacial: !isShowBtn}">
      	 <dl v-if="isShowNum">
    ...
    <div class="cathectic">
      <dl>
        <dt @click="isShowPan=false">
            <span v-for="(item,index) in NameList" v-if="$route.params.id == item.id" :data-name=" item.name" id="my_item_name">
              {{item.name}}
            </span>
