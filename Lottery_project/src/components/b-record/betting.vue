<template>
  <div class="betting-content" >
    <div class="betting-head">
      <ul>
        <li class="betting-head1">
          <a  onclick="history.go(-1)" >
            <img src="../../images/Return-@2x.png" alt="">
          </a>
        </li>
        <li class="betting-head2 title" @click="isShow=!isShow">
          <span>{{order_name}}</span>
          <i v-show="!isShow" class="triangle-down"></i>
          <i v-show="isShow" class="triangle-up"></i>
        </li>
        <li class="betting-head3">
          <span @click="onRefresh(false)"><Icon type="refresh" style="font-size: 0.4rem;float: right;padding: 0.25rem 0 0.25rem 0.58rem" color="#fff"></Icon></span>
        </li>
      </ul>
    </div>
    <!--下拉显示-->
    <transition name="fade">
      ...
      <div v-show="isShow" class="header-is-active" @click ="isShow = false">

        <div class="header-modal-content">
          <ul>
            <li v-for="(items, key) in order_name_list" :class="{red: items.val==order_id}" @click="chkOrd(items.val,items.name)">{{items.name}}</li>
          </ul>
        </div>
      </div>
    </transition>
    ...
    <div class="betting" :style="{'-webkit-overflow-scrolling': scrollMode}">
      ...
      <!--<mt-loadmore :top-mothod="loadTop" :bottom-method="loadMore" :bottom-all-loaded="allLoaded" ref="loadmore" :auto-fill="false" @top-status-change="handleTopChange">-->
        <dl class="betting-list" v-for="(lotteryList,key) in lotteryList"  @click="gotoBetting(lotteryList,key)">
          <dt class="betting-list-img">
            ...
            <img v-if="lotteryList.sub_orders[0].lot_type=='极速PK拾'||lotteryList.lot_type=='极速PK拾'" src="../../images/icon/jspk10.png" alt="">
            <!--天津时时彩-->
            ...
            <img v-if="lotteryList.sub_orders[0].lot_type=='分分时时彩'||lotteryList.lot_type=='分分时时彩'" src="../../images/icon/minuteslot.png" alt="">
            <!--幸运28-->
            ...
            <img v-if="lotteryList.sub_orders[0].lot_type=='极速六合彩'||lotteryList.lot_type=='极速六合彩'" src="../../images/icon/jslhc.png" alt="">
            <!--广西快三-->
          ...
          <dd class="betting-list-state">
            <dl>
              ...
              <dd v-if="order_id!=3" :class="{blue:lotteryList.is_winning_all==2,red:lotteryList.is_winning_all==1,orange:lotteryList.is_winning_all==0}">{{lotteryList.is_winning_all | transWinningStatus}}</dd>
              <dd v-if="order_id==3" :class="{orange:lotteryList.total_executed==0}">{{lotteryList.total_executed | transExecuted}}</dd>
            </dl>
          </dd>
        </dl>
      </v-scroll>
  ...
  export default {
    ...
    data() {
      return {
        ...
        allLoaded: false,
        page: 1,
        order_id: this.$route.params.orderID || 0,
        order_name: this.$route.params.orderID==1?'待开奖':'全部订单',
        order_name_list: [
          {name: '全部订单', val: 0},
          {name: '待开奖', val: 1},
          {name: '已中奖', val: 2},
...
//        this.$refs.loadmore.onBottomLoaded()
      ...
      bet_records(loadflag) { // 投注记录 this.user.token
        mt_loading_open();
        setTimeout(() => {
          mt_loading_close()
        },5000)
        ...
        requestOpt.reqGet(url, sessionStorage.getItem('token'), (res) => {
          if(res.data.status === 1) {
            if(res.data.data.list.length <= 0){
              ...
              //  this.$el.querySelector('.yo-scroll').classList.add('bg_img');
            }else{
              //  this.$el.querySelector('.yo-scroll').classList.remove('bg_img');
      ...
      gotoBetting(obj, key) {
        ...
        if (this.order_id == 3) {
          ...
          console.log(obj.is_winning_all)
          if (obj.is_winning_all==0) {
            this.$router.push({path: '/betting-chase/'+this.lotteryKey[key]+'/'+this.lucky_info})
          } else {
            this.$router.push({path: '/betting-detail/'+this.lotteryKey[key]})
          }
        }
...
// 所有订单
 ...
 .betting dl.betting-list .betting-list-state dl dd.orange{
   color #fda90b
 }
</style>
