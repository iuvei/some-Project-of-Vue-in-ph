			...
			<div class="basketDetails-top-content">
				<table>
					<tr>
						...
						<th style="width:1rem">上半场</th>
						<th style="width:1rem">下半场</th>
						<th style="width:1rem">加时</th>						
						<th style="width:1.9rem">总计</th>
					</tr>
					<tr>
						<td>{{home}}</td>
						<td class="yello">{{score.hHalfScore}}</td>
						<td>{{score.hSHalfScore}}</td>
						<td class="yello">{{score.hOverTimeScore}}</td>
						<td class="bg">{{score.homeScore}}</td>
					</tr>
					<tr>
						<td>{{away}}</td>
						<td class="yello">{{score.aHalfScore}}</td>
						<td>{{score.aSHalfScore}}</td>
			...
			<transition name="fade">
				<div class="main-content" v-show="meObj.show" v-if="obj[2]" @click="bet($event)">
					<div class="main-item"  v-for="(item, key, index) in obj" :key=key v-if="starObj[key]">
						<div class="main-item-title" v-if="key!='show'">
							<span v-if="key!=8&&key!=7&&key!=12">{{item[0].payTypeName}}</span>
							<span v-if="key==8">波胆</span>
							<span v-if="key==7">进球数区间</span>
							<span v-if="key==12">半场/全场（胜负）</span>
							<i :data-num='[key,item[0].id]' :class="{active : starObj[key]}"></i>
						</div>
						<div class="main-item-content" v-if="key==1||key==4">
							<div v-for="i in item" class="main-item-list">
								<div class="list-real" :data-data="[item[0].id,item[0].realRate1,item[0].payTypeName,1]" style="width: 38%">
									<span>{{home}}</span>
									<p><span>{{item[0].realRate1}}</span></p>
								</div>
								<div class="list-real" :data-data="[item[0].id,item[0].realRate2,item[0].payTypeName,3]" style="width: 24%;justify-content: center;">
									<p>平<span>{{i.realRate2}}</span></p>
								</div>
								<div class="list-real" :data-data="[item[0].id,item[0].realRate3,item[0].payTypeName,2]" style="width: 38%">
									<span>{{away}}</span>
									<p><span>{{item[0].realRate3}}</span></p>
								</div>
							</div>
						</div>
						<div class="main-item-content" v-if="key==5||key==2">
							<div v-for="i in item" class="main-item-list">
						...
						<div class="main-item-content" v-if="key==6||key==3||key==59||key==60||key==48||key==61||key==62||key==49">
							<div v-for="i in item" class="main-item-list">
								<div class="list-real" :data-data="[i.id,i.realRate1,i.payTypeName,1]">
									<span>大</span>
						...
						<div class="main-item-content" v-if="key==8">
							<div class="main-item-list bodan">
								<div v-for="(i,index) in item"  class="list-real" :data-data="[i.id,i.payRate,i.name]">
									<span>{{i.name}}</span>
									<p><span>{{i.payRate}}</span></p>
								</div>
							</div>
						</div>
						<div class="main-item-content" v-if="key==14||key==13||key==57||key==58||key==56||key==76||key==77">
							<div class="main-item-list">
								<div class="list-real" :data-data="[item[0].id,item[0].realRate1,item[0].payTypeName,1]">
									<span>{{key==57||key==58||key==56||key==76||key==77 ? "是" : "单"}}</span>
									<p><span>{{item[0].realRate1}}</span></p>
								</div>
								<div class="list-real" :data-data="[item[0].id,item[0].realRate3,item[0].payTypeName,2]">
									<span>{{key==57||key==58||key==56||key==76||key==77 ? "否" : "双"}}</span>
									<p><span>{{item[0].realRate3}}</span></p>
								</div>
							</div>
						</div>
						<div class="main-item-content" v-if="key==73">
							<p class="content-title">
								<span>{{home}}</span><span>和局</span><span>{{away}}</span>
							</p>
							<div class="main-item-list" style="background: #fff;">
								<div style="width: 33.3%" v-for="(i,index) in item">
									<div class="list-real" :data-data="[n.id,n.payRate,n.name]"  v-for="n in i" style="width: 100%;padding:0 .1rem;">
										<span>{{n.name}}</span>
										<p><span>{{n.payRate}}</span></p>
									</div>
									<div class="list-real"style="width: 100%" v-if="index==1"></div>
									<div class="list-real"style="width: 100%" v-if="index==1"></div>
								</div>								
							</div>
						</div>
						<div class="main-item-content" v-if="key==12">
							<p class="content-title">
								<span>{{home}}</span><span>和局</span><span>{{away}}</span>
							</p>
							<div class="main-item-list" style="flex-wrap: wrap; flex-direction:column;height: 1.56rem;background: #fff;">
								<div v-for="i in item"  class="list-real" :data-data="[i.id,i.payRate,i.name]" style="width: 33.3%;padding: 0 .1rem;">
									<span>{{i.name}}</span>
									<p><span>{{i.payRate}}</span></p>
								</div>
							</div>
						</div>
						<div class="main-item-content" v-if="key==7">
							<div class="main-item-list">
								<div  v-for="i in item" class="list-real" :data-data="[i.id,i.payRate,i.name]" style="width: 25%;padding: 0 .16rem;">
									<span>{{i.name}}</span>
									<p><span>{{i.payRate}}</span></p>
								</div>
							</div>
	...
	export default {
		...
		mounted() {
			...
			if(getUserInfo()){
				...
				let url = `pay/foucePay/score/matchID/${matchID}`
				let url2 = `pay/foucePay/matchID/${matchID}`
				requestOpt.reqTokenGet(url,getUserInfo().authToken, res => {
					...
					if(res.data.status&&res.data.data) {
						...
						this.starLen += num;
						Object.assign(this.starObj, obj);
					}
				}, err => {
					Toast('网络链接失败，请稍后再试')
					Indicator.close();
				});
				requestOpt.reqTokenGet(url2,getUserInfo().authToken, res => {
					Indicator.close();
					if(res.data.status&&res.data.data) {
						let data = res.data.data;
						let obj = {};
		...
		methods: {	
			...
			getData(){
				...
				let url2 = `pay/score/matchID/${id}`;
				let url3 = `match/part/matchID/${id}`;				
				let len = 0;
					...
					if(res.data.status&&res.data.data) {
						...
						for(let i = 0;i < data.length; i++){
							...
							if(data[i].type == 1 || data[i].type == 3){
								let arr = data[i].list[0].items;
								for(let i = 0;i < arr.length; i++){
									if(!obj[arr[i].payTypeID]){
										obj[arr[i].payTypeID] = [];
									}									
									obj[arr[i].payTypeID].push({
										id:arr[i].id,
								...
								obj[8] = [];
								for(let i = 0;i < arr.length; i++){	
									for(let n = 0;n < arr[i].items.length; n++){			
										obj[8].push({
											id:arr[i].items[n].id,
											payRate:arr[i].items[n].payRate,
											name:arr[i].items[n].name,								
										})	
									}	
								}	
							}else{
...
<style scoped>
	...
	.main-item-list.bodan{
		flex-wrap: wrap;
	}
	.main-item-list.bodan div{
		width: 20%;
		padding: 0 .1rem;
	}
	
	.main-item-list.bodan div:last-child{
		width: 40%;
	}
	...
	.basketDetails-top-content{
		...
		background: url(../../images/football_bg11.png);
