<template>
	<div class="rechargeRecord" >
		<back title="充值记录"></back>
            ...
            <div class="rechargeRecord-content" >
                <mt-loadmore :top-method="loadTop" v-infinite-scroll="loadBottom" infinite-scroll-distance="40"  infinite-scroll-disabled="allLoaded" infinite-scroll-immediate-check="false" ref="loadmore">
                    <div class="rechargeRecord_mid" >
                        <mt-tab-container v-model="active_index" swipeable >
                            <mt-tab-container-item id="1">
                                <div class="recharge_cont_item" v-if="concat_list" v-for="(item,index) in concat_list" :key="index" >
                                    <p><span>充值金额 <i>{{ item.orderID }}</i></span> <strong>{{item.typeName}}</strong></p>
                                    <h3><span>+{{item.amount}}</span> <strong><i>{{item.state == 0 ? '待审核' : item.state == 1 ? '成功' : '失败' }}</i>{{item.createTime}}</strong></h3>
                                </div>
                            ...
                            <mt-tab-container-item id="3" >
                                <div class="recharge_cont_item" v-if="concat_list" v-for="(item,index) in concat_list" :key="index" >
                                    <p  v-if="item"><span>充值金额 <i >{{ item.orderID }}</i></span> <strong>{{item.typeName}}</strong></p>
                                    <h3 v-if="item"><span>+{{item.amount}}</span> <strong><i>{{item.state == 0 ? '待审核' : item.state == 1 ? '成功' : '失败' }}</i>{{item.createTime}}</strong></h3>
                                </div>
...
<script type="text/ecmascript-6">
	...
	export default {
		...
		data() {
			return {
                switch_tab_text:['全部', '充值成功', '失败' , '待审核'],
                active_index: '1',
                allLoaded:false,
                curr_page : 1,
                total_page : 1,
		...
		methods: {
            ...
            loadBottom(){
                ...
                this.curr_page++;
                requestOpt.reqTokenGet('charge/record/type/' + this.active_index +'/page/' + this.curr_page +'/count/10',this.user.authToken,
                        (res)=>{
                            if(res.data.status){
...
<style>
    ...
    .recharge_tab ul li.active{
        border-bottom: 2px solid #ee7f49;
        color:#ee7f49;
        font-size: 0.3rem;
    }
    .rechargeRecord-content .mint-tab-container .mint-tab-container-item{
        min-height: 10.6rem;
    }
    .rechargeRecord-content{
        margin-top:1.68rem;
    }
    .recharge_cont_item{
        padding: 0 .3rem;
    ...
    .recharge_cont_item p span{
        color: #000;
        font-size: 0.28rem;
    }

    .recharge_cont_item p span i{
        font-size: 0.25rem;
        color: #929292;
    }
    .recharge_cont_item p strong{
        font-size: 0.28rem;
        color: #000;
        font-weight: normal;
    }

    .recharge_cont_item h3{
        width: 100%;
        display: flex;
        justify-content: space-between;
    }
    .recharge_cont_item h3 span{
        font-size: 0.28rem;
        color: #fd0000;
    }

    .recharge_cont_item h3 strong {

        font-weight: normal;
        color:#929292;
    }
    .recharge_cont_item h3 strong i{
        font-size: 0.25rem;
        margin-right: 0.3rem;
        /* color:#929292; */
    }
