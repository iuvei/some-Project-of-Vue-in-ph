	...
	<div class="noteRecord">
		<div class="noteRecord-heard">
			...
			<div class="noteRecord-heard-tab">
				<a class="active">足球</a>
				<router-link to="/noteRecordBall">篮球</router-link>
			</div>
			<a></a>
		</div>
		...
		methods: {
			...
			getData() {
				...
				if(this.scoreIndex == 1) {
					url = `payBall/recent/type/${this.tabIndex}`;
				} else if(this.scoreIndex == 2) {
					url = 'payBall/unchecked';
				} else {
					let startDate = this.starText.replace(/[-]/g, "");
					let endDate = this.endText.replace(/[-]/g, "");
					url = `payBall/history/startDate/${startDate}/endDate/${endDate}`;
				}
