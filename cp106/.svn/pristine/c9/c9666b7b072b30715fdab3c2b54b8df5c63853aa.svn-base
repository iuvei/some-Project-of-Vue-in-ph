        ...
        <div class="notice-right-l">
          ...
          <p>
            <marquee scrollamount="2">
              ...
              <span style="margin-right:5px;"  v-for="(item,index) in notice" :key="index">{{item.content}}</span>
            </marquee>
          </p>
        </div>
...
<script>
  ...
  export default {
    ...
    data () {
      return {
        notice: JSON.parse(sessionStorage.getItem('topNotice')),
        app_config: JSON.parse(sessionStorage.getItem('app_config'))
      }
    },
    methods: {
      onLine() {
